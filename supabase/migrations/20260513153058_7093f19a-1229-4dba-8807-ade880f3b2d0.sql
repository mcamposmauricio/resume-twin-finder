...
CREATE INDEX IF NOT EXISTS idx_job_applications_favorite ON public.job_applications(job_posting_id) WHERE is_favorite;

DROP FUNCTION IF EXISTS public.get_talent_applications(uuid, text);
DROP FUNCTION IF EXISTS public.get_talent_pool(uuid, text, uuid[], text, boolean, integer, timestamp with time zone, integer, integer);

CREATE OR REPLACE FUNCTION public.get_talent_applications(p_user_id uuid, p_email text)
 RETURNS TABLE(id uuid, job_posting_id uuid, job_title text, job_status text, triage_status text, status text, created_at timestamp with time zone, resume_url text, resume_filename text, form_data jsonb, form_fields jsonb, is_favorite boolean)
 LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.get_talent_pool(p_user_id uuid, p_search text DEFAULT NULL::text, p_job_ids uuid[] DEFAULT NULL::uuid[], p_triage_status text DEFAULT NULL::text, p_has_resume boolean DEFAULT NULL::boolean, p_min_applications integer DEFAULT NULL::integer, p_date_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_only_favorites boolean DEFAULT NULL::boolean, p_page integer DEFAULT 1, p_page_size integer DEFAULT 50)
 RETURNS TABLE(email text, name text, phone text, total_applications bigint, latest_date timestamp with time zone, latest_job_title text, latest_job_posting_id uuid, latest_application_id uuid, latest_triage text, latest_status text, has_resume boolean, latest_resume_url text, latest_resume_filename text, score integer, is_favorite boolean, total_count bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
AS $function$
...
BEGIN
  ...
  FROM (
    ...
    HAVING
      ...
      AND (p_only_favorites IS NULL OR p_only_favorites = false OR bool_or(ja.is_favorite) = true)
  ) sub;
  ...
  WITH talent_agg AS (
    SELECT
      ...
      bool_or(ja.is_favorite) AS t_is_favorite,
      (
        CASE
  ...
  ORDER BY ta.t_is_favorite DESC, ta.t_score DESC, ta.t_latest_date DESC
